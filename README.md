...
Updated: Biweekly, on website. Set up as a BigLocal News cron job. 

Some fields overlap but  the individual WARN reports contain more specific information. Currently data from individual reports cannot be set up as a cron job. The individual WARN reports came as Excel (.xlsx) files. 

Data processing was done with Python and relied primarily on the openpyxl package, because the position of data headers and their corresponding data fields were not uniformly formatted. (For example: the number of affected employees would be one cell below the header, while the date of notice received would be in the cell on the right of the header. Additionally, each report was a worksheet within a workbook; data for any given year was broken into multiple workbooks, alphabetically.
